...
func TestKeyFrom(t *testing.T) {
	...
	assert.ErrorContains(err, "key length mismatch, expected 32, got 3")
	assert.Nil(k)
...
func TestCheckKey(t *testing.T) {
	...
	assert.ErrorContains(CheckKey(k), `key length mismatch, expected 32, got 4`)
...
func TestMACer(t *testing.T) {
	...
	macer, err := New(k)
	assert.ErrorContains(err, `key length mismatch, expected 32, got 4`)
	assert.Nil(macer)
