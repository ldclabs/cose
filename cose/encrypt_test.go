	...
	for _, tc := range []struct {
	...
	}{
		{
			...
			map[key.IntKey]any{
				...
				key.ParamKid: []byte("meriadoc.brandybuck@buckland.example"),
				key.ParamAlg: key.AlgES256,
				key.ParamCrv: key.CrvP256,
				key.ParamX:   key.Base64Bytesify("mPUKT_bAWGHIhg0TpjjqVsP1rXWQu_vwVOHHtNkdYoA"),
				key.ParamY:   key.Base64Bytesify("8BQAsImGeAS46fyWw5MhYfGTT0IjBpFw2SS34Dv4Irs"),
			},
			map[key.IntKey]any{
				key.ParamKty: key.KtyEC2,
				key.ParamKid: []byte("meriadoc.brandybuck@buckland.example"),
				key.ParamAlg: key.AlgES256,
				key.ParamCrv: key.CrvP256,
				key.ParamX:   key.Base64Bytesify("Ze2loSV3wrroKUN_4zhwGhCqo3Xhu1td4QjeQ5wIVR0"),
				key.ParamY:   key.Base64Bytesify("HlLtdXARY_f55A3fnzQbPcm6hgr34Mp8p-nuzQCE0Zw"),
				key.ParamD:   key.Base64Bytesify("r_kHyZ-a06rmxM3yESK84r1otSg-aQcVStkRhA-iCM8"),
			},
		...
		gcmkey, err := aesgcm.KeyFrom(key.AlgA128GCM, tc.cek)
		...
		require.NoError(t, err, tc.title)
		rp := &Recipient{
			Protected: Headers{HeaderLabelAlgorithm: -25}, // ECDH-ES + HKDF-256
			Unprotected: Headers{
				HeaderAlgEphemeralKey: ck,
				HeaderLabelKeyID:      []byte("meriadoc.brandybuck@buckland.example"),
			},
		}
